    ...
    return (
        <div className="p-8 max-w-screen-lg w-full">
            ...
            {zaps.map(z => <div key={z.id} className="flex border-b border-t py-4">
                ...
                <div className="flex-1 flex"><img src = {z.trigger.type.image} className="w-[30px] h-[30px]" /> {z.actions.map(x => <img src={ x.type.image} key={x.id} className="w-[30px] h-[30px]" /> )}</div>
                <div className="flex-1">{z.id}</div>
                <div className="flex-1">Nov 13, 2023</div>
                <div className="felx-1"> {`${HOOK_URL}/hooks/catch/${z.userId}/${z.id}`}</div>
